...
3. Implement the smallest `blazium_4.8`-safe pattern (peer, then `@rpc`, then spawn/sync).
...
Do not invent classes, flags, or CLI verbs. Pin Blazium 0.8.x (Godot 4.8.x fork, branch `blazium_4.8`). Prefer JustAMCP, Autowork, or `blazium-cli` over unverified editor clicks. Grok `code_execution` is not multiplayer evidence.
